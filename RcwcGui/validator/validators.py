...
class BaseValidator(wx.Validator):
    ...
    def Clone(self):
        return BaseValidator(self.flag)
    ...
    def OnChar(self, event):
        ...
        if (key < wx.WXK_SPACE or key > 255) and not key in (wx.WXK_DELETE, wx.WXK_BACK):
            event.Skip()
            return

        if self.flag == ALPHA_ONLY and (chr(key) in string.ascii_letters or key == wx.WXK_DELETE):
            event.Skip()
            return
        ...
        if self.flag == DIGIT_ONLY and chr(key) in string.digits:
            event.Skip()
            return
        if self.flag == NUMBER_ONLY and chr(key) in number:
            tc = self.GetWindow()
            val = tc.GetValue()
            if chr(key) == '.' and str.find(val, '.') >= 0:
                return
            event.Skip()
            return
...
class IntegerValidator(BaseValidator):
    def __init__(self):
        super(IntegerValidator, self).__init__(flag=DIGIT_ONLY)


class FloatingPointValidator(BaseValidator):
    def __init__(self):
        super(FloatingPointValidator, self).__init__(flag=NUMBER_ONLY)


class AsciiLettersValidator(BaseValidator):
    def __init__(self):
        super(AsciiLettersValidator, self).__init__(flag=ALPHA_ONLY)
...
class TestValidatorPanel(wx.Panel):
    def __init__(self, parent):
        ...
        fgs.Add(wx.TextCtrl(self, -1, "", validator=BaseValidator(ALPHA_ONLY)))
        ...
        fgs.Add(label, 0, wx.ALIGN_RIGHT | wx.CENTER)
        fgs.Add(wx.TextCtrl(self, -1, "", validator=BaseValidator(DIGIT_ONLY)))
